...
Compiler Layer 主要由 `js_token_bin` 根模块暴露稳定门面；CLI、wasm 和 Web Workbench 都复用这组接口。
...
Runtime Layer 主要由 `js_token_bin::executor` 和 `js_token_bin::compiler` 中的执行 facade 暴露。

#### 推荐 Rust 执行接口

- `execute_source(source: &str) -> Result<String, String>`
  - 编译并执行 JS 源码。
- `execute_bytes(bytes: &[u8], seed: &str) -> Result<String, String>`
  - 按 Seed 校验并执行 bytecode bytes。
...
- `js_execute_bytes_with_seed(bytes: &[u8], seed: &str) -> Result<String, String>`
  - 按 Seed 校验并执行 bytes。
- `js_encoding_seed_from_rows(opcodes, operand_tags, constant_tags, bytes) -> Result<String, String>`
  - 从三张编码表的名称排列生成 Seed。
- `js_encoding_seed_for_seed_and_bytes(seed: &str, bytes: &[u8]) -> Result<String, String>`
  - 从配置 Seed 和 bytes 生成配对运行 Seed。
- `js_encoding_rows_from_seed(seed: &str) -> Result<Vec<String>, String>`
  - 从 Seed 恢复编码表名称排列。
