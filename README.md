...
该脚本会依次运行 Rust 测试、构建 wasm、执行 `node tests/E2E.js` 和 `node tests/Differential.js`，覆盖编译、编码、随机 seed、执行器和 Node/d8/VM 可观测输出对比。
...
`--log` 会把完整终端输出写入 `tests/issues/<YY-MM-DD:HH:mm:ss>/log.txt`；`--error=0..5` 用来控制异常归档范围：`1` 只归档 internal failures，`2` 增加 compile errors，`3` 增加 runtime errors，`4` 增加 expected JS runtime errors，`5` 再增加 skipped。归档结果按状态写入 `tests/issues/<YY-MM-DD:HH:mm:ss>/<status>/`，摘要写入 `tests/issues/<YY-MM-DD:HH:mm:ss>/errors.log`。
...
```text
cargo test -> npm run build:wasm -> npm test -> npm run differential -> npm run fuzz:quick
```
...
```text
npm run verify -> npm run fuzz:quick
```
