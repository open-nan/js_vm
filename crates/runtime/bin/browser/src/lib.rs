//! 浏览器环境运行时 wasm 入口。
//!
//! 这个 crate 负责把 runtime core 暴露给 H5 页面。它只假设宿主是浏览器 JS 环境：
//! externals 由调用方按数组传入，属性访问和函数调用通过 `JsHostBridge` 走 `Reflect`。
//!
//! 常用入口：
//! - `js_execute_void_bytes_with_seed`：执行 script，忽略返回值。
//! - `js_execute_value_bytes_with_seed`：执行 script，返回 `JsValue`。
//! - `js_execute_module_bytes_with_seed`：执行 module，返回模块 namespace。
...
pub fn js_execute_bytes(
    ...
    execute_bytes_with_seed_and_limits(
        ...
        seed,
        externals.into_vec(),
        option_usize(&options, "max_call_depth", DEFAULT_MAX_CALL_DEPTH),
        option_usize(
            &options,
            "max_recursive_call_depth",
            DEFAULT_MAX_RECURSIVE_CALL_DEPTH,
        ),
        option_usize(&options, "max_execution_steps", DEFAULT_MAX_EXECUTION_STEPS),
    )
}
...
///
/// 该入口只在 `runtime-profile` feature 打开时导出，用于分析大包启动时的热 opcode、
/// 热 pc 和 HostBridge 调用占比。
...
impl JsVmDebugSession {
    ...
    pub fn result(&self) -> Result<JsValue, String> {
        let value = self
            .inner
            .result_value()
            .ok_or_else(|| "debug session has not completed".to_string())?;
        value_to_js_value(&value, &JsHostBridge::empty()).map_err(|err| err.to_string())
    }
}

fn option_usize(options: &JsValue, key: &str, default: usize) -> usize {
...
fn normalize_js_externals(module: &BytecodeModule, mut externals: Vec<JsValue>) -> Vec<JsValue> {
    // 调用方可以显式传 externals；没传满时浏览器包只做常见全局对象兜底。
    // 真正的调用/属性语义仍由 runtime core 的 HostBridge 统一处理。
...
fn browser_global_external(name: &str) -> Option<JsValue> {
    // 这里不是硬编码 native call，只是把全局根对象放进 extern slot。
    // 例如 `console.log` 会先得到 `console`，后续 `.log()` 仍走 Reflect.get/apply。
    match name {
